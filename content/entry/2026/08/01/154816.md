---
title: "はてなブログからGitHub Pagesへ移行しました"
date: 2026-08-01T15:48:16+09:00
categories:
  - "ブログ"
  - "Hugo"
  - "GitHub Actions"
...
<!--
TODO: ここに冒頭の動機（これまではてなブログにお世話になってきたこと、
生成AIで時代が変わりつつある中での心境など）を書く。
書き終えたら front matter の draft を false にしてマージすると公開されます。
-->
...
- はてなブログから GitHub Pages（Hugo）へ移行しました
...
手順5で「ワークフローを書き換える必要がない」と書きましたが、落とし穴があります。

`baseURL` はビルド時に決まるため、**カスタムドメインを設定しただけでは、既にデプロイ済みの成果物は古い `baseURL` のまま**です。私の場合、DNS切り替え直後のサイトはCSSも画像も全て404でした。

```
https://blog.morihaya.tech/<repo>/css/bundle.min...css   404
https://blog.morihaya.tech/<repo>/images/fotolife/...    404
```

ワークフローを手動実行して再ビルドすれば直ります。カスタムドメインを設定したら、**忘れずに一度デプロイし直してください**。
...
- フィードURL（後述）
- `morihaya.hatenablog.com` 宛のリンク
  - 現時点でははてな側がカスタムドメインへ301を返してくれているため新サイトに着地しますが、解約後は失われる見込みです
...
[morihaya/Hatena-Blog-Workflows](https://github.com/morihaya/Hatena-Blog-Workflows)
